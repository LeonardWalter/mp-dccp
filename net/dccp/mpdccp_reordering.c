...
struct rcv_buff *mpdccp_init_rcv_buff(struct sock *sk, struct sk_buff *skb, struct mpdccp_cb *mpcb)
{
	...
	printk("DEQ(%p): receive %llu", mpcb, (u64)rb->oall_seqno);
	return rb;
}
...
void mpdccp_owdd_update(struct mpdccp_reorder_path_cb* pcb, u32 timestamp)
{
	...
	measurement = jiffies_to_msecs(jiffies) - timestamp;
    if(!pcb->onewayd) {
		...
		//printk("DEQ(%p): now %u, send %u, diff %u, this_owd %u, s_owd %u delta %i", pcb, jiffies_to_msecs(jiffies), timestamp, measurement, pcb->onewayd, (int)(delta_ts/8));
    }
}
